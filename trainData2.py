...
data_file = glob.glob('/local/data/chaitanya/DR/classification/dr_class_seperated_data/trainData2/*.jpeg')
...
for f in (data_file):
    ...
    base = os.path.basename("/local/data/chaitanya/DR/classification/dr_class_seperated_data/trainData2/" + f)
    fileName = os.path.splitext(base)[0]
    trainLabel.append(trainLabels.loc[trainLabels.image==fileName, 'level'].values[0])
    i += 1
    
np.save('trainData2.npy',trainData)
np.save('trainLabel2.npy', trainLabel)
